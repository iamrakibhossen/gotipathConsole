...
use WHMCS\Module\Server\gotipathConsole\MyMetricsProvider;
 ...
 * example file, the filename is "gotipathConsole" and therefore all
 * functions begin "gotipathConsole_".
 *
...
const CLIENT_ID = '9946487a-7b48-4093-bc85-af4a8cca8177';
const CLIENT_SECRET = 'w1DehLslIsAxjvR6hxB6ctryPWcSadSHKKlj7ghg';
const ACCESS_TOKEN_URL = 'http://api.gotipath.test/oauth/token';
const STATUS_API_URL = 'http://api.gotipath.test/v1/change/status/customer-package';
const BASE_URL = 'http://api.gotipath.test';
...
function gotipathConsole_MetricProvider($params)
{
    return new MyMetricsProvider($params);
}

function gotipathConsole_UsageUpdate($params)
{
    // var_dump($params);
    ...
    //Update the tblhosting table for the given server to update the disk usage and bandwidth usage values for the account in WHMCS
    try {
 ...
 * @return array
 */
function gotipathConsole_MetaData()
{
    return array(
        'DisplayName' => 'Gotipath Console',
        ...
        // 'MetricProviderUniqueIdentifierField' => 'username',
 ...
 * @return array
 */
function gotipathConsole_ConfigOptions()
{
    return array(

        // Gotipath Package Name
        // 'package' => array(
        //     'Type' => 'dropdown',
        //     'Options' => array(
        //         'option1' => 'Starter',
        //         'option2' => 'Advanced',
        //         'option3' => 'Professional',
        //     ),
...
/**
 * Function to obtain an access token from the authorization server.
 *
 ...
 * @return string
 */
function gotipathConsole_getAccessToken($clientId, $clientSecret, $accessTokenUrl)
{
    $data = [
 ...
 * @return string
 */
function gotipathConsole_makeRequest($apiUrl, $accessToken, $data, $method)
{
    $curl = curl_init();
 ...
 */
function gotipathConsole_CreateAccount(array $params)
{
    try {
        //  Obtain an access token from console backend
        $accessToken = gotipathConsole_getAccessToken(CLIENT_ID, CLIENT_SECRET, ACCESS_TOKEN_URL);

        $data = array(
            'billing_order_id' => $params['model']->orderid, 'type' => 'active'
        );
        // $data = array('billing_order_id' => 87);
        //  Make a request to the protected endpoint
        $response = gotipathConsole_makeRequest(STATUS_API_URL, $accessToken, $data, 'POST');
        // var_dump($response);
        // die;
        // Output the response
        // echo $response;
 ...
 * @return string "success" or an error message
 */
function gotipathConsole_SuspendAccount(array $params)
{
    try {
        ...
        $data = array(
            'billing_order_id' => $params['model']->orderid, 'type' => 'deactive'
        );
        // $data = array('billing_order_id' => 87);
        //  Make a request to the protected endpoint
        $response = gotipathConsole_makeRequest(STATUS_API_URL, $accessToken, $data, 'POST');
 ...
 * @return string "success" or an error message
 */
function gotipathConsole_UnsuspendAccount(array $params)
{
    try {
 ...
 * @return string "success" or an error message
 */
function gotipathConsole_TerminateAccount(array $params)
{
    try {
        //  Obtain an access token from console backend
        $accessToken = gotipathConsole_getAccessToken(CLIENT_ID, CLIENT_SECRET, ACCESS_TOKEN_URL);
 ...
 * @return string "success" or an error message
 */
function gotipathConsole_ChangePassword(array $params)
{
    try {
 ...
 * @return string "success" or an error message
 */
function gotipathConsole_ChangePackage(array $params)
{
    try {
        // Call the service's change password function, using the values
        // provided by WHMCS in `$params`.
 ...
 * @return string "success" or an error message
 */
function gotipathConsole_Renew(array $params)
{
    try {
 ...
 * @return array
 */
function gotipathConsole_TestConnection(array $params)
{
    try {
 ...
 *
 * @see gotipathConsole_buttonOneFunction()
 *
 * @return array
 */
function gotipathConsole_AdminCustomButtonArray()
{
    return array(
 ...
 * @return array
 */
function gotipathConsole_ClientAreaCustomButtonArray()
{
    // return array(
 ...
 *
 * @see https://developers.whmcs.com/provisioning-modules/module-parameters/
 * @see gotipathConsole_AdminCustomButtonArray()
 *
 * @return string "success" or an error message
 */
function gotipathConsole_buttonOneFunction(array $params)
{
    try {
 ...
 *
 * @see https://developers.whmcs.com/provisioning-modules/module-parameters/
 * @see gotipathConsole_ClientAreaCustomButtonArray()
 *
 * @return string "success" or an error message
 */
function gotipathConsole_actionOneFunction(array $params)
{
    try {
        // Call the service's function, using the values provided by WHMCS in
        // `$params`.
 ...
 *
 * @see https://developers.whmcs.com/provisioning-modules/module-parameters/
 * @see gotipathConsole_AdminServicesTabFieldsSave()
 *
 * @return array
 */
function gotipathConsole_AdminServicesTabFields(array $params)
{
    try {
        ...
        return array(
            ...
            // 'Something Editable' => '<input type="hidden" name="gotipathConsole_original_uniquefieldname" '
            //     . 'value="' . htmlspecialchars($response['textvalue']) . '" />'
            //     . '<input type="text" name="gotipathConsole_uniquefieldname"'
            //     . 'value="' . htmlspecialchars($response['textvalue']) . '" />',
        );
    } catch (Exception $e) {
        // Record the error in WHMCS's module log.
 ...
 *
 * @see https://developers.whmcs.com/provisioning-modules/module-parameters/
 * @see gotipathConsole_AdminServicesTabFields()
 */
function gotipathConsole_AdminServicesTabFieldsSave(array $params)
{
    // Fetch form submission variables.
    $originalFieldValue = isset($_REQUEST['gotipathConsole_original_uniquefieldname'])
        ? $_REQUEST['gotipathConsole_original_uniquefieldname']
        : '';

    $newFieldValue = isset($_REQUEST['gotipathConsole_uniquefieldname'])
        ? $_REQUEST['gotipathConsole_uniquefieldname']
        : '';
 ...
 * @return array
 */
function gotipathConsole_ServiceSingleSignOn(array $params)
{
    try {
 ...
 * @return array
 */
function gotipathConsole_AdminSingleSignOn(array $params)
{
    try {
 ...
 * @return array
 */
function gotipathConsole_ClientArea(array $params)
{
