 ...
 */
function hook_gotipathConsole_clientedit(array $params)
{
    try {
 ...
 */
add_hook('ClientEdit', 1, 'hook_gotipathConsole_clientedit');
    ...
    if (!is_null($menu->getChild('Services'))) {
        ...
        $menu->getChild('Services')
            ->addChild(
                'Gotipath Console',
                array(
                    'uri' => 'clientarea.php?action=services&module=gotipathConsole',
                    'order' => 15,
                )
            );
    }
...
add_hook('ClientAreaSecondarySidebar', 1, function ($secondarySidebar)
{
    ...
    if (!is_null($secondarySidebar->getChild('My Services Actions'))) {
        ...
        $customPanel = $secondarySidebar->addChild('Gotipath Console');
